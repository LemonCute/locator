			...
			if(USART_RX_STA&0x4000)//接收到了0x0d
				{
					if(Res!=0x0a)	
						...
						USART_RX_STA++;	
							}												//重新查询 0x0d			//USART_RX_STA=0;//接收错误,重新开始				
					else USART_RX_STA|=0x8000;	//接收完成了 
				}
			else 														//还没收到0X0D
				{	
				if(Res==0x0d)		
					{ 	USART_RX_STA|=0x4000;	USART_RX_BUF[USART_RX_STA&0X3FFF]=Res ;   USART_RX_STA++;	}
				else
					{
					USART_RX_BUF[USART_RX_STA&0X3FFF]=Res ;
					USART_RX_STA++;
       ...
       for(i=0;i<num;i++)
        {            
						while( USART_GetFlagStatus(USART1,USART_FLAG_TC)!= SET)  //等待发送完成。   检测 USART_FLAG_TC 是否置1；   
										;
							 USART_SendData(USART1,*send_array++);            //通过库函数  发送数据;   //见库函数 P359 介绍  
        }    
}
